...
import {
  adminUpdateAdhkarTargetAction,
} from "@/lib/admin/adhkarActions";
import { MIN_ADHKAR_TARGET, MAX_ADHKAR_TARGET, type AdhkarTargetRow } from "@/lib/settings/adhkarShared";
...
function AdminTargetRow({
  ...
  function handleSave() {
    ...
    startTransition(async () => {
      try {
        await adminUpdateAdhkarTargetAction({
          userId,
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 py-3">
      ...
      <div className="flex items-center gap-3">
        <label className="sr-only" htmlFor={`admin-adhkar-target-${row.practiceItemId}`}>
          {`${label} — ${copy.currentTarget}`}
        </label>
        <input
          id={`admin-adhkar-target-${row.practiceItemId}`}
          type="text"
          inputMode="numeric"
          pattern="[0-9]*"
          value={value}
...
/**
 * Renders in /admin/users/[id]. Lets an admin raise/lower a user's
 * default adhkar target on their behalf — this only affects days the
 * user hasn't logged yet; every past daily_practice_logs row keeps the
 * target_value it was created with (see 0007_adhkar_custom_targets.sql),
 * so editing here never rewrites a historical report.
 */
export function AdminAdhkarTargetsSection({
  userId,
  rows,
}: {
  userId: string;
  rows: AdhkarTargetRow[];
}) {
      ...
      {morning.length > 0 && (
        <div className="mt-5">
          ...
          <div className="mt-1 divide-y divide-border">
            {morning.map((row) => (
              <AdminTargetRow key={row.practiceItemId} userId={userId} row={row} locale={locale} />
            ))}
          </div>
        </div>
      )}
